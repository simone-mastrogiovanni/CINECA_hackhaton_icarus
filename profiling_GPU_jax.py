...
Ntot = [1e4,5e4,1e5,3e5,5e5,1e6,3e6,5e6,1e7]
Nsamples = 5000
Nrep = 101
Ninj = 5000
timing_flag = 'timing/GPU_jax.txt'
...
prior*=jnp2onp(icarogw.conversions.source2detector_jacobian(onp2jnp(data['injections/redshift'][()]),cosmo_ref)) # Add jacobian to convert prior in detector frame
...
for nt in Ntot:
    ...
    if timing_flag:
        fp.write('{:e}\t{:e}\t{:e}\n'.format(nt,np.median(timing[1:]),np.std(timing[1:])))
